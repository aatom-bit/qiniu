...
const filePath = path.resolve(__dirname, "resource/iat/16k_10.pcm");
// const filePath = path.resolve(__dirname, "resource/iat/16k_10.mp3");
...
async function Begin() {
    const wsUrl = getAuthUrl(hostUrl, apiKey, apiSecret);
    const ws = new WebSocket(wsUrl);

    ws.on('open', () => {
        console.log("WebSocket connected. Start sending audio...");
        const pcmStream = fs.createReadStream(filePath, {
            highWaterMark: 1280,
            encoding: null,
        });
